Ext.define('Admin.view.batch.BatchController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.batch_controller',
    ...
    _initSearchParams:function(){
        var spinfoses = Ext.ComponentQuery.query("form[name='batch_search_form']");
        ...
        params.batchno = formParams.batchno;
        return params;
    },
    ...
    onuseraddClick:function(){
        ...
        var params= {
            ...
            this.setCurrentView('batchadd',params);
         /*   var me = this,
   ...
   onusersaveClick: function(view) {
    ...
    var formparams = form.getValues();
     ...
     just.showWaitingDlg("正在上传,请稍候...");
           form.form.doAction('submit',{
               submitEmptyText:false,
               url:just.getUrl('/sys/ascode/importbatch.action'),
               method : 'POST',
               success : function(cmp,response, opts){
                    var jsonObj = Ext.JSON.decode(response.response.responseText);
                     just.hideWaitingDlg(jsonObj.info,true);
...
_onDownloadtemple: function(cmp, cell, index, row, event, raw, eOpts) {
        window.location.href = just.getUrl(Ext.String.format("/download/fdtxt.action?path={0}",'/template/系统_产品打包入库模版.xml'));
